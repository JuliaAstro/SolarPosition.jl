...
# materialise it at precision T. Instead we extract an exact integer day count since the
# J2000.0 epoch plus the milliseconds into that day (also an exact integer), and only the
# small intra-day fraction (in [0, 1)) is carried in T. The integer day part is exact in any T
# that can hold it (Float32 is exact up to 2^24 days ≈ 46 000 yr), so precision is preserved
# for BigFloat while the magnitude stays small enough for Float32/Float16.
#
# J2000_EPOCH_MS is anchored at noon (JD 2451545.0 == 2000-01-01T12:00), so the day count
# equals jd - 2451545.0 (days since J2000 noon) with no half-day offset.
...
# Same day-count as `julian_day_j2000` but kept as exact integer day + [0, 1) fraction
