...
@inline function _usno_kernel(
        dt::DateTime, δt::T, sin_lat, cos_lat, longitude, gmst_option,
        ::Type{T},
    ...
    # geocentric apparent ecliptic longitude [deg]
    ...
    ha = (gast - ra) * 15.0 + longitude

    # solar elevation [deg]
    elevation = asind(cosd(ha) * cosd(δ) * cos_lat + sind(δ) * sin_lat)

    # azimuth [deg]
    azimuth =
        rad2deg(atan(-sind(ha), (tand(δ) * cos_lat - sin_lat * cosd(ha))))
...
function _solar_position(obs::Observer{T}, dt::DateTime, alg::USNO) where {T <: AbstractFloat}
    ...
    return _usno_kernel(dt, δt, obs.sin_lat, obs.cos_lat, obs.longitude, alg.gmst_option, T)
end
...
function _solar_position!(
    ...
    gmst_option = alg.gmst_option

    @inbounds for i in eachindex(dts, pos)
        pos[i] = _usno_kernel(dts[i], δt, sin_lat, cos_lat, longitude, gmst_option, T)
    end
    return pos
end

_resolve_refraction(::USNO, ::DefaultRefraction, ::Type{T}) where {T} = NoRefraction()
